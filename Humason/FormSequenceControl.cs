...
namespace Humason
{
    public partial class FormSequenceBuilder : Form
    {
        ...
        {
            if (newlen > ProgressBar.Maximum) newlen = ProgressBar.Maximum;
        ...
        private void SBInitialConfiguration()
        {
            ...
            if (currentTarget != null)
            {
                ...
                tPlan.SequenceStartTime = StartTimeBox.Value;
                if (tPlan.TargetAdjustEnabled) AdjustedTargetLabel.Visible = true;
                else AdjustedTargetLabel.Visible = false;
            }
        }
        ...
        {//Update form fields with the content of a new target plan from the session control file
            TargetPlan tPlan = new TargetPlan(FormHumason.openSession.CurrentTargetName);
            if (tPlan.TargetName == "Default") TargetBox.Text = "";
            else TargetBox.Text = tPlan.TargetName;
